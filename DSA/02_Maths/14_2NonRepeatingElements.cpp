...
vector<int> twoUniqueNums(vector<int> nums) {
   ...
   // zor of whole array every repeating element will be remoes xOrr will have
   // both unique elemts;
   for (int i = 0; i < nums.size(); i++) {
      xorr ^= nums[i];
   }

   // checking for first set bit because two different setbit 1^0=1 so on first
   // setbit at both numbers bit is different are differrent (finding first
   // setbit (1))
   int checking = xorr;
   int i = 0;
   int nosetbit = 0;
   ...
   // base on the fist set bit dividing array in two part (if th ith bith of an
   // element is =1 is goes to set bit else not set bit)
...
vector<int> twoUniqueNums2(vector<int> nums) {
   ...
   //  so in place of arrays we can do this we can sepecificaly take the zor of
   //  each elememt that has its bits set and for the nuber that has its bit
   //  unset
