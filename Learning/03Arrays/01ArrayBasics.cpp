...
int main(){
    ...
    //you can make an array of any type and its store in contigious manner means an array of int (int arr[3]) will be sotred with the gap of 4 bytes because the one int takes 1 byte to store the value.

   // and array index star from zero (arr[o]) to store and retrive use 0 index . array are fixed in nature one you inisilized the array you cant change it . 
